...
class Mapset {
    ...
    functionDraw() {
      ...
      this.features = this.data.features.map(({geometry, properties, type})=>{
        ...
        return {x:x, y:y, value : properties.stayTime};
      });
    ...
    //   //console.log(_sw);
        ...
        const layers  = [
            new ScatterplotLayer({
                ...
                filterRange: [showMin, showMax],
                getPosition: d => [d.x,d.y],
                getRadius: (d) => {
                    const colorLimit = this.maxValue*0.001;                                           
                    ...
                    if (this.currentZoom >= 14) {          
                        ...
                        radius = Math.pow(r, 0.5) / ((zoomNowHere-12));
                    } else {   
                ...
                updateTriggers: {
                    ...
                    getRadius: [zoomNowHere],
                    getFillColor : [showGreater, highlightGreater],
                    getLineColor : [showGreater, highlightGreater]
                },
                visible : true,
    ...
    getColor(d, isBorder) {
        ...
        let color0 =  d.value>sizeLimit? 
                        d3.color(mapColor(Math.log(d.value)+3)) 
                        : d3.color(mapColor(Math.log(d.value)));
    ...
    static loadJson(fileName) {
      ...
      const points = results.data.map((d) => {
            const [x,y] = [(d.gridx+BASE_UNIT/2), (d.gridy+BASE_UNIT/2)];
            return turf.point([x,y], {stayTime : d.dayAvgstayTime}); 
          });
...
const data1 = Mapset.loadJson("./11350_휴일_생활권_40대미만.tsv");
const data2 = Mapset.loadJson("./11350_휴일_생활권_40대이상.tsv");
...
mapset1.title = "노원구 40대미만";
mapset2.title = "노원구 40대이상";
...
sliderInstance.update({
    ...
    //prettify_separator: ",",
    prettify : (n) => ( Math.pow(Math.exp(1),n).toFixed(1)),
    postfix: " h",
    onStart: (sliderData) => { 
...
$('#lockGrid').on('ifChecked', function(event){
    lockGrid = true;
    mapsetArr.forEach(d=>d.render()); 
    });
    $('#lockGrid').on('ifUnchecked', function(event){
    lockGrid = false;
});
